...
def main():
  ...
  feature_extractor = CLIPFeatureExtractor.from_pretrained('openai/clip-vit-base-patch32', subfolder="feature_extractor")
  ...
  if True:
    ...
    dataset = {'train': CustomDatasetForImages(args.train_data_dir, preprocess_train, True), 'val': CustomDatasetForImages(
        args.train_data_dir, preprocess_train, False)}
  ...
  def collate_fn(examples):
    generated = torch.stack([example["pixel_values"][0] for example in examples])
    generated = generated.to(memory_format=torch.contiguous_format).float()

    canonical = torch.stack([example["cond_pixel_values"][0] for example in examples])
    canonical = canonical.to(memory_format=torch.contiguous_format).float()

    input_ids = torch.stack([example["input_ids"][0] for example in examples])
    return {"pixel_values": generated, "cond_pixel_values": canonical, "input_ids": input_ids}
  ...
  def compute_val_loss():
    ...
    for step, batch in enumerate(val_dataloader):
      ...
      encoder_hidden_states = encoder_hidden_states + cond_latents.unsqueeze(1)
  ...
  for epoch in range(first_epoch, args.num_train_epochs):
    ...
    for step, batch in enumerate(train_dataloader):
      ...
      # pdb.set_trace()
      # import torchvision
      # target = torchvision.transforms.ToPILImage()(((batch['pixel_values'][0] + 1) * 127.5).type(torch.uint8))
      # target.save('target.jpeg')
      # src = torchvision.transforms.ToPILImage()(((batch['cond_pixel_values'][0] + 1) * 127.5).type(torch.uint8))
      # src.save('src.jpeg')
      ...
      if global_step % 500 == 0:
        ...
        accelerator.log(val_loss, step=global_step)
